...
>
> By default, Windows recreates the `/etc/hosts` file everytime you start Bash on Ubuntu on Windows. To ensure that your changes are actually persisted, remove the line that says that the file is *automatically generated by WSL* (Notice: You have to edit the `/etc/hosts` file with `sudo` rights).
:::
